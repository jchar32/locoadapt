...
def v_residual(nd_signals: np.ndarray) -> np.ndarray:
...
def waveform_summary_metrics_over_trials(
    ...
    for t, trial in enumerate(list(nd_waveforms.keys())):
        ...
        vres_out_ = v_residual(data)
        vres_.append(vres_out_)

        vres_mean_.append(np.nanmean(vres_out_, axis=0))
...
def lewek_phase_variability(nd_waveforms: np.ndarray):
    ...
    phi_mean_sd = np.mean(phi_sd, axis=0)
